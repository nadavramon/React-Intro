...
3. For **each selected task, in order**, build the subagent dispatch from its equipment tags, then dispatch a **foreground** subagent:
   ...
   - Pass only that task's steps + the plan path. The subagent implements, then runs the relevant check (`npm run test:run`, `tsc -b`, or `npm run lint` as fits the task). If the task commits in a format-on-save repo (Prettier), tell the subagent to format **only the files its task touched** (`prettier --write <paths>`) before it commits — never the repo-wide format script, which clobbers the user's uncommitted in-flight edits (a `pnpm format` once reformatted a dirty CLAUDE.md that wasn't the task's to touch). Format-on-save never fires for programmatic edits, so otherwise unformatted commits surface only at the final `format:check`.
...
4. After each task returns:
   - Tick its box (`- [ ]` → `- [x]`) in the plan file. **Tick by resolved line number**: `grep -n` the step line first, then `sed -i '<N>s/- \[ \]/- [x]/'` on that address (or the Edit tool on that exact line). Never tick by title pattern — step titles repeat across tasks ("Green, format, commit") and a pattern once ticked the wrong task's box. Never an `awk`/whole-file rewrite either, which makes the harness re-inject the entire plan into context on every tick (pure token waste).
...
6. When the selected range is done, update `docs/superpowers/INDEX.md` status (`In progress`, or `Done` once all boxes are ticked). Before marking a feature `Done`, **verify the way CI does**: run the full `/check` (lint + typecheck + test), not a hand-picked subset (the `tsc` hook alone gives false confidence); if the feature changed dependencies, regenerate the lockfile clean (`rm -rf node_modules package-lock.json && npm install`) before committing — a local `npm ci` passes on macOS but Linux CI fails on stale `@emnapi` peers. When a feature reaches `Done`, suggest the user run `/retro <feature>` to capture lessons before moving on — a suggestion, not a forced step.
